...
- (void)itemsWillAppearFromButton:(UIButton *) button withFrame:(CGRect)frame inView:(UIView *)view {
	...
	float angle = (arc>=360)?(360/itemCount):((itemCount>1)?(arc/(itemCount-1)/2):0.0f);
	...
	if ([self.delegate respondsToSelector:@selector(buttonSizeForRadialMenu:)]) {
		self.buttonSize = [self.delegate buttonSizeForRadialMenu:self];
	}
	...
	while (currentItem <= itemCount) {
		...
		CGRect frame = CGRectMake(centerX-self.buttonSize*0.5f, centerY-self.buttonSize*0.5f, self.buttonSize, self.buttonSize);
